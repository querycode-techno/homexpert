...
const subscriptionHistorySchema = new Schema({
  ...
  status: {
    type: String,
    enum: ['pending', 'active', 'paused', 'expired', 'cancelled', 'refunded'],
    default: 'pending',
    index: true
  },
  isActive: {
  ...
  payment: {
    ...
    paymentMethod: {
      type: String,
      enum: ['credit_card', 'debit_card', 'upi', 'bank_transfer', 'wallet', 'cash', 'admin_credit'],
      required: true
    },
  ...
  history: [{
    action: {
      type: String,
      enum: ['purchased', 'activated', 'paused', 'resumed', 'upgraded', 'downgraded', 'renewed', 'cancelled', 'expired', 'refunded'],
      required: true
    },
  ...
  notifications: {
    lowLeadsAlert: { type: Boolean, default: false },
    expiryReminder: { type: Boolean, default: false },
