...
import { getDurationInDays, getDurationMonths, validateDuration } from '@/lib/utils/subscriptionUtils';
...
const subscriptionPlanSchema = new Schema({
  ...
  duration: {
    type: String,
    required: true,
    // Format: "N-month" where N is 1-24 (validated in pre-save hook)
    match: [/^\d+-month$/, 'Duration must be in format "N-month" (e.g., "3-month")']
  },
  durationInDays: {
    type: Number,
    required: true
...
subscriptionPlanSchema.virtual('monthlyEquivalent').get(function() {
  const months = getDurationMonths(this.duration);
  const effectivePrice = this.discountedPrice || this.price;
...
subscriptionPlanSchema.pre('save', function(next) {
  ...
  if (this.isModified('duration')) {
    const validation = validateDuration(this.duration, 24);
    if (!validation.isValid) {
      return next(new Error(validation.error));
    }
    
    // Calculate duration in days dynamically
    this.durationInDays = getDurationInDays(this.duration);
  }
