...
// When user send a leads booking request or booking a lead then this notification send to all vendor and admin
export async function POST(req) {
    await connectDB();
  
    const userType = "user"; // as per your requirement
    ...
    try {
      ...
      const recipientDocs = users.map(user => ({
        ...
        userId: user._id,
        userType: user.type,
        deliveryStatus: user.fcmToken ? "pending" : "failed", // Use "failed" instead of "no_token"
